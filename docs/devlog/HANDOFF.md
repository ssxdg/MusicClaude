...
> 在主目录 `<repo>` 跑 `git pull && npm ci && npm run deploy:build`;重建后新 JS bundle 应为
...
> WARN-only — a missing `<corpus>/modern-poetry` now **throws** so a rebuild can't silently drop the 508 modern
...
bundle at `<backup-dir>/shiyun-ALL-branches-backup.bundle` (restore: `git clone <bundle>`).
...
- [docs/ARCHITECTURE.md](../ARCHITECTURE.md) — layers, what's stable vs replaceable, data flow.
- [docs/ENGINE_API.md](../ENGINE_API.md) — engine + engineApi surface, invariants, MSB convention.
- [docs/DATA_CONTRACT.md](../DATA_CONTRACT.md) — static-asset schemas, corpus, dynasty taxonomy + normalization map.
- [docs/FRONTEND_GUIDE.md](../FRONTEND_GUIDE.md) — **rebuild contract**: the 4 stable interfaces a new frontend uses (load.ts / engineApi / poetPosition / store), interaction model, locked direction notes.
- [docs/PIPELINE.md](../PIPELINE.md) — how the data + lexicon are built.
- [docs/DEVLOG.md](../DEVLOG.md) — running development diary (newest first): per-round commits + what changed + verify.
- [docs/DEPLOY.md](../DEPLOY.md) — static deploy (nginx + brotli + the poems/ Range gotcha).
...
- `<corpus>\Werneror-Poetry` — all-dynasties corpus (MIT). Used by `pipeline/build-data.mjs`.
- `<corpus>\modern-poetry` — yuxqiu/modern-poetry 新诗 set (Apache-2.0). Also read by `build-data.mjs`.
- `<corpus>\Pingshui_Rhyme.json` — 平水韵 (charlesix59, MIT). Used by `pipeline/build-lexicon.mjs`.
...
- ✅ **Deploy kit** (`deploy/nginx.conf` + `deploy/precompress.mjs` + [docs/DEPLOY.md](../DEPLOY.md),
...
10. ✅ **Modern 新诗 poets** — imported yuxqiu/modern-poetry (Apache-2.0, `<corpus>/modern-poetry`):
